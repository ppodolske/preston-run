...
function setStatus(key,online){const el=document.getElementById(key+'Status');el.className='status '+(online?'online':'issue');el.querySelector('span').textContent=online?'Online':'Unavailable';document.getElementById('today'+(key==='dose'?'Dose':'Parks')).textContent=online?'Online and ready':'Currently unavailable';}
async function statuses(){try{const r=await fetch('/api/status',{cache:'no-store'});if(!r.ok)throw new Error('status');const x=await r.json();setStatus('dose',!!x.dose?.online);setStatus('parks',!!x.parks?.online);}catch(e){setStatus('dose',false);setStatus('parks',false);}}
weather();statuses();setInterval(statuses,120000);
</script>
</body></html>`;

async function probe(url){const started=Date.now();try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),3500);const r=await fetch(url,{method:'GET',redirect:'follow',signal:controller.signal,headers:{'user-agent':'Preston.run status/0.3'}});clearTimeout(timer);return {online:r.status<500,status:r.status,ms:Date.now()-started};}catch(e){return {online:false,status:null,ms:Date.now()-started};}}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/api/status'){const [dose,parks]=await Promise.all(APPS.map(a=>probe(a.url)));res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({checkedAt:new Date().toISOString(),dose,parks}))}
