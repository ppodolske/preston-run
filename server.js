...
async function probe(url){const started=Date.now();try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),3500);const r=await fetch(url,{method:'GET',redirect:'follow',signal:controller.signal,headers:{'user-agent':'Preston.run status/0.2'}});clearTimeout(timer);return {online:r.status<500,status:r.status,ms:Date.now()-started};}catch(e){return {online:false,status:null,ms:Date.now()-started};}}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/health'){res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({ok:true,version:VERSION,apps:APPS.map(a=>a.url),weather:'Open-Meteo / Sydney'}))}
