...
const CATEGORY_KEYS=new Set(CATEGORY_OPTIONS.map(([value])=>value).filter(Boolean));

function filterHref(filter,category){if(filter==='active'&&category)return `/life-admin?filter=${encodeURIComponent(category)}`;return filter&&filter!=='active'?`/life-admin?filter=${encodeURIComponent(filter)}`:'/life-admin';}
function categoryFilter(category){return `<div class="category-filter"><label for="category">Category</label><select id="category" name="category" onchange="window.location.href=this.value?'/life-admin?filter='+encodeURIComponent(this.value):'/life-admin'">${CATEGORY_OPTIONS.map(([value,text])=>`<option value="${esc(value)}"${String(value)===String(category||'')?' selected':''}>${esc(text)}</option>`).join('')}</select><noscript><span class="meta">Use the category links in the URL if JavaScript is disabled.</span></noscript></div>`;}
...
function renderLifeAdminPage({lifeItems=[],tasks=[],people=[],filter='active',category='',needsAttention=[],comingUp=[],flash=null}={}){
  const legacyCategory=CATEGORY_KEYS.has(filter)?filter:'';
  const effectiveCategory=category||legacyCategory;
