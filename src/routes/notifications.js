...
function parseOffsets(value){return String(value||'').split(',').map(v=>v.trim()).filter(Boolean).map(Number);}
function settingsFromForm(form){return validateReminderSettings({...defaults(),birthday_offsets:parseOffsets(form.get('birthday_offsets')),renewal_offsets:parseOffsets(form.get('renewal_offsets')),deadline_offsets:parseOffsets(form.get('deadline_offsets')),appointment_offsets:parseOffsets(form.get('appointment_offsets')),trip_offsets:parseOffsets(form.get('trip_offsets'))});}
