...
const {renderCalendarViewPage}=require('../pages/calendar-view');
const {getCalendarMonthWindow,buildCalendarMonth}=require('../domain/calendars');

const STATE_COOKIE='calendar_oauth_state';

function pathMatches(path){return path==='/calendar'||path==='/settings/calendars'||path==='/settings/calendars/google/connect'||path==='/settings/calendars/google/callback'||path==='/settings/calendars/apple/connect'||path==='/settings/calendars/sync'||path==='/settings/calendars/google/disconnect'||path==='/settings/calendars/apple/disconnect'||/^\/settings\/calendars\/sources\/[^/]+\/toggle$/.test(path);}
...
async function handleCalendarsRoute(req,res,context){
  ...
  if(req.method==='GET'&&url.pathname==='/calendar'){await renderFullCalendar(res,supabase,user,deps,url);return true;}
  if(req.method==='GET'&&url.pathname==='/settings/calendars'){await renderSettings(res,supabase,user,deps,url);return true;}
  ...
  if(req.method==='GET'&&url.pathname==='/settings/calendars/google/callback'){
    ...
    try{
      ...
      stage='calendar_discovery';const calendars=await deps.googleProvider.listCalendars({accessToken:tokens.accessToken});
      stage='account_identity';const identity=deps.googleProvider.getAccountIdentity(calendars);
      stage='credential_encryption';const ciphertext=deps.encryptCredential({refreshToken:tokens.refreshToken},keyFor(config,deps));
      stage='connection_save';const connection=await deps.upsertCalendarConnection(supabase,user.id,{provider:'google',account_external_id:identity.externalId,account_label:identity.label,credential_ciphertext:ciphertext,status:'connected',last_error:null});
      stage='source_save';await deps.replaceDiscoveredCalendarSources(supabase,user.id,connection.id,normalizedSources(calendars));redirectCookie(res,'/settings/calendars?connected=google',stateCookie('',config,{clear:true}));
  ...
  if(url.pathname==='/settings/calendars/apple/connect'){
    ...
    try{const credentials={email,appSpecificPassword};const validated=await deps.appleProvider.validateAppleCredentials(credentials);const calendars=validated&&Array.isArray(validated.calendars)?validated.calendars:await deps.appleProvider.listCalendars(credentials);const ciphertext=deps.encryptCredential(credentials,keyFor(config,deps));const connection=await deps.upsertCalendarConnection(supabase,user.id,{provider:'apple',account_external_id:email.toLowerCase(),account_label:email,credential_ciphertext:ciphertext,status:'connected',last_error:null});await deps.replaceDiscoveredCalendarSources(supabase,user.id,connection.id,normalizedSources(calendars));redirect(res,'/settings/calendars?connected=apple');}catch(_error){redirect(res,'/settings/calendars?error=apple');}return true;
  }
  ...
  if(url.pathname==='/settings/calendars/sync'){try{await deps.syncCalendars({supabase,userId:user.id,now:new Date(),credentialKey:keyFor(config,deps),googleConfig:{clientId:config.googleCalendarClientId,clientSecret:config.googleCalendarClientSecret}});redirect(res,'/settings/calendars?synced=1');}catch(_error){redirect(res,'/settings/calendars?error=sync');}return true;}
