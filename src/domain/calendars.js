...
function normalizeCalendarEvent(input={}){
  const out={
    provider_event_id:requiredText(input.provider_event_id,'provider ID'),
    occurrence_key:requiredText(input.occurrence_key,'occurrence key'),
    series_id:input.series_id?String(input.series_id):null,
    title:requiredText(input.title,'title'),
    all_day:Boolean(input.all_day),
    status:input.status||'confirmed',owner_response:input.owner_response||'unknown',
    time_zone:input.time_zone?String(input.time_zone):null,location:input.location?String(input.location):null,
    external_url:input.external_url?String(input.external_url):null,provider_updated_at:input.provider_updated_at||null
  };
...
function sortCalendarDigestEvents(events=[],window){
  const dayStart=new Date(window.startOfToday).getTime();
  function bucket(e){
    if(e.all_day)return allDayOverlapsDate(e,window.today)?0:2;
    return new Date(e.starts_at).getTime()<new Date(localDateTimeToInstant(window.tomorrow).toISOString()).getTime()?1:3;
  }
  return [...events].sort((a,b)=>{
    const ba=bucket(a),bb=bucket(b);if(ba!==bb)return ba-bb;
    if(a.all_day&&b.all_day){const c=String(a.start_date).localeCompare(String(b.start_date));if(c)return c;return String(a.title).localeCompare(String(b.title));}
    if(!a.all_day&&!b.all_day){const c=new Date(a.starts_at)-new Date(b.starts_at);if(c)return c;return String(a.title).localeCompare(String(b.title));}
...
module.exports={SYDNEY_TZ,getCalendarSyncWindow,getMorningCalendarWindow,shouldRunCalendarSync,normalizeCalendarEvent,isCalendarEventDigestEligible,sortCalendarDigestEvents,isConnectionStale};
