...
async function runMorningSummary({supabase,userId,now=new Date(),pushTransport,deps={}}){const d={...defaultDeps,...deps};if(!userId)throw new Error('userId is required');if(!pushTransport||typeof pushTransport.send!=='function')throw new Error('pushTransport is required');const {today,items}=await eligibleMorningItems({supabase,userId,now,deps:d});if(!items.length)return{sent:false,count:0};const key=`daily-summary:${today}`;const existing=await d.getOccurrence(supabase,userId,key);if(existing&&['sent','acknowledged'].includes(existing.status))return{sent:false,count:items.length,deduplicated:true};const reminder=existing||await d.upsertOccurrence(supabase,userId,{entity_type:'summary',entity_id:today,reminder_class:'daily_summary',occurrence_key:key,target_date:today,effective_trigger_at:now.toISOString(),status:'pending',deep_link:'/',policy_source:'default'});const payload={title:'preston.ai morning summary',body:summaryBody(items),url:'/',tag:`preston-daily-${today}`};const delivered=await deliver({supabase,userId,reminder,payload,pushTransport,deps:d});return{sent:delivered>0,count:items.length,deliveries:delivered};}
