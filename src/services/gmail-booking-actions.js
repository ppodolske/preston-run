...
function buildGmailBookingActions({supabase,userId,bookingData=bookingDataDefault,bookingSourceData=bookingSourceDataDefault,bookingLegData=bookingLegDataDefault,tripData=tripDataDefault,gmailData={recordGmailActivity},reviewData=null,proposeTripLink=proposeTripLinkDefault,ruleVersion='gmail-booking-actions-v0.14.0'}={}){
  ...
  return {
    async processBooking({source,candidate,facts=[],trips=[],relatedBookings=[]}){
      ...
      const archivedAssignedTrip=existing&&existing.trip_id?(trips||[]).find(t=>t.id===existing.trip_id&&t.archived_at):null;
      const input=bookingInput(candidate);
      if(tripLock.locked||archivedAssignedTrip)delete input.trip_id;
      ...
      if(bookingLegData&&typeof bookingLegData.upsertBookingLegFromGmail==='function'){
        for(const leg of Array.isArray(candidate.legs)?candidate.legs:[]){
          await bookingLegData.upsertBookingLegFromGmail(supabase,user,booking.id,leg,{source:'gmail',source_record_id:source.id,extractor_version:ruleVersion});
        }
      }
      ...
      if(archivedAssignedTrip){
        return{booking,created,linkDecision:{kind:'link',tripId:archivedAssignedTrip.id,score:100,reasons:['existing_archived_trip_assignment'],proposedTrip:null},facts};
      }

      const activeTrips=(trips||[]).filter(t=>!t.archived_at);
      const linkDecision=proposeTripLink({subjectType:'booking',subject:{...candidate,...booking},trips:activeTrips,relatedBookings});
