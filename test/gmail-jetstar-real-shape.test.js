...
assert.equal(diagnostics.flightRowLooseMeridiemCount,2);
assert.equal(diagnostics.dateBeforeServiceCount,2);
assert.equal(diagnostics.timeBeforeServiceCount,2);
assert.equal(diagnostics.serviceBeforeTimeCount,0);
...
const spacedMeridiem=text.replace(/(\d{1,2}:\d{2})(am|pm)/gi,'$1 $2');
const spacedDiagnostics=jetstarDiagnostics(spacedMeridiem,{maxLength:120000});
assert.equal(spacedDiagnostics.flightRowCount,0,'strict row matcher should demonstrate the production-sensitive meridiem spacing gap');
assert.equal(spacedDiagnostics.flightRowLooseMeridiemCount,2,'loose diagnostic should still recognize both departure rows');
assert.equal(spacedDiagnostics.dateBeforeServiceCount,2);
assert.equal(spacedDiagnostics.timeBeforeServiceCount,2);
