...
const {createLifeItem,updateLifeItem}=require('../src/data/life-admin');
...
(async()=>{
  ...
  b=builder();supabase={from(name){assert.equal(name,'tasks');return b;}};await createTask(supabase,{id:'u1'},{title:'Pay fee',status:'open',priority:'urgent',linked_life_item_id:'l1',linked_person_id:''});const taskInsert=b.calls.find(x=>x[0]==='insert')[1];assert.equal(taskInsert.user_id,'u1');assert.equal(taskInsert.linked_life_item_id,'l1');assert.equal(taskInsert.linked_person_id,null);
  b=builder();supabase={from(){return b;}};await updateTask(supabase,{id:'u1'},'t1',{title:'Pay fee',status:'completed',priority:'normal'});assert.ok(b.calls.some(x=>x[0]==='eq'&&x[1]==='user_id'&&x[2]==='u1'));
