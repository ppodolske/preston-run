...
let handleCalendarsRoute,providerStatusCode;
try{({handleCalendarsRoute,providerStatusCode}=require('../src/routes/calendars'));}catch(error){assert.fail(`Calendars route module required: ${error.message}`);}
assert.equal(providerStatusCode(new Error('Google Calendar calendar discovery failed (403)')),403);
assert.equal(providerStatusCode(new Error('Google Calendar calendar discovery failed')),null);
assert.equal(providerStatusCode(null),null);
...
(async()=>{
 ...
 const diagnostics=[];const failingDeps={...deps,reportCalendarError:(provider,stage,error)=>diagnostics.push({provider,stage,status:providerStatusCode(error)}),googleProvider:{...deps.googleProvider,listCalendars:async()=>{throw new Error('Google Calendar calendar discovery failed (403)');}}};
 r=res();await handleCalendarsRoute(req('GET',`/settings/calendars/google/callback?code=code-2&state=${encodeURIComponent(stateValue)}`,'',{cookie:`calendar_oauth_state=${encodeURIComponent(stateValue)}`}),r,{supabase:ownerSupabase(),config,calendarDeps:failingDeps});assert.equal(r.status,302);assert.equal(r.headers.location,'/settings/calendars?error=google');assert.deepEqual(diagnostics,[{provider:'google',stage:'calendar_discovery',status:403}]);
