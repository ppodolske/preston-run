...
const {listTrips,getTrip,createTrip,createGeneratedTrip,updateTrip,deleteTrip,listSegments,getSegment,createSegment,updateSegment,deleteSegment}=require('../src/data/trips');
const {listBookings,getBooking,createBooking,updateBooking,updateBookingFromGmail,deleteBooking}=require('../src/data/bookings');

function builder(resultData=[]){const calls=[];let written=null;const b={calls,resultData,select(v){calls.push(['select',v]);return b;},insert(v){written=v;calls.push(['insert',v]);return b;},update(v){written=v;calls.push(['update',v]);return b;},delete(){calls.push(['delete']);return b;},eq(k,v){calls.push(['eq',k,v]);return b;},is(k,v){calls.push(['is',k,v]);return b;},order(k,o){calls.push(['order',k,o]);return b;},limit(v){calls.push(['limit',v]);return b;},single(){return Promise.resolve({data:{id:'new',...written},error:null});},maybeSingle(){const base=Array.isArray(resultData)?resultData[0]||null:resultData;return Promise.resolve({data:written?{...(base||{}),...written}:base,error:null});},then(resolve){resolve({data:Array.isArray(resultData)?resultData:resultData?[resultData]:[],error:null});}};return b;}
