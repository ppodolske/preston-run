...
(async()=>{
  ...
  r=response();await handleGmailRoute(request('GET','/me/settings/gmail/scan-status'),r,{supabase:ownerDb(),config:routeConfig,gmailDeps:{listRecentScans:async()=>[{id:'scan1',status:'running',processed_count:4,relevant_count:1,ignored_count:3,review_items_created_count:0,started_at:'2026-09-13T00:00:00Z'}]}});assert.equal(r.status,200);assert.match(r.headers['content-type'],/application\/json/);const status=JSON.parse(r.body);assert.equal(status.latestScan.id,'scan1');assert.equal(status.latestScan.status,'running');assert.equal(status.latestScan.processed_count,4);
