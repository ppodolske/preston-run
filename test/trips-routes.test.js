...
function store(owner=true){
  const data={
    trips:[{id:'t1',user_id:'u1',title:'Chicago',status:'upcoming',start_date:'2026-10-01',end_date:'2026-10-10',notes:null},{id:'t2',user_id:'u1',title:'Other',status:'planning',start_date:null,end_date:null,notes:null}],
...
(async()=>{
  ...
  s=store(true);r=res();await handleTripsRoute(req('GET','/trips'),r,{supabase:s,config});assert.equal(r.status,200);assert.match(r.body,/Chicago/);assert.equal(r.headers['cache-control'],'private, no-store');
  r=res();await handleTripsRoute(req('GET','/trips/t1'),r,{supabase:s,config});assert.equal(r.status,200);assert.match(r.body,/Qantas/);assert.match(r.body,/Pack/);
  ...
  r=res();await handleTripsRoute(req('POST','/trips','title=Milwaukee&status=upcoming&start_date=2026-10-11&end_date=2026-10-15'),r,{supabase:s,config});assert.equal(r.status,302);assert.match(r.headers.location,/\/trips\/new-trips/);assert.equal(s.data.trips.length,3);assert.equal(s.data.trips[2].user_id,'u1');
  ...
  r=res();await handleTripsRoute(req('GET','/bookings/missing/edit'),r,{supabase:s,config});assert.equal(r.status,404);
