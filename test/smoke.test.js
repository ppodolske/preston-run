...
const login=renderLoginPage();for(const forbidden of ['railway.com','Weather','Calendar','Personal','Holidays','Reminders','Overdue','Today','Life Admin','Trips','Notifications','/notifications','/calendar','/settings/calendars','Google Calendar','Apple / iCloud Calendar','GOOGLE_CALENDAR_CLIENT_ID','GOOGLE_CALENDAR_CLIENT_SECRET','CALENDAR_CREDENTIAL_KEY','credential_ciphertext','app_specific_password','VAPID_PRIVATE_KEY','SUPABASE_SERVICE_ROLE_KEY'])assert.ok(!login.includes(forbidden),`login leaked ${forbidden}`);
