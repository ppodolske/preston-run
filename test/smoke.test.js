...
assert.equal(pkg.version,'0.12.2');assert.equal(VERSION,'0.12.2');assert.equal(PRODUCT_NAME,'preston.ai');
...
const home=renderHomePage({user:{},upcomingBirthdays:[],upcomingLifeItems:[],overdueItems:[],todayItems:[],calendar:{personal:[],holidays:[],reminders:[]},upcomingTrips:[]});for(const expected of ['id="condition"','Weather','Calendar','Personal','Holidays','Reminders','Overdue','Today','Birthdays','Life Admin','Trips','/people','/life-admin','/trips','/calendar','/settings/calendars','/me/settings/gmail','app-launcher','Log out','v0.12.2'])assert.ok(home.includes(expected),`home missing ${expected}`);
