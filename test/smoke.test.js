...
assert.equal(pkg.version,'0.12.4');assert.equal(VERSION,'0.12.4');assert.equal(PRODUCT_NAME,'preston.ai');
...
const home=renderHomePage({user:{},upcomingBirthdays:[],upcomingLifeItems:[],overdueItems:[],todayItems:[],calendar:{personal:[],holidays:[],reminders:[]},upcomingTrips:[]});for(const expected of ['id="condition"','Weather','Calendar','Personal','Holidays','Reminders','Overdue','Today','Birthdays','Life Admin','Trips','/people','/life-admin','/trips','/calendar','/settings/calendars','/me/settings/gmail','app-launcher','Log out','v0.12.4'])assert.ok(home.includes(expected),`home missing ${expected}`);
...
for(const file of ['src/routes/people.js','src/routes/life-admin.js','src/routes/trips.js','src/routes/notifications.js','src/routes/calendars.js','src/pages/calendars.js','src/pages/calendar-view.js','src/data/reminders.js','src/data/calendars.js','src/domain/calendars.js','src/services/reminder-engine.js','src/services/calendar-sync.js','src/services/calendar-digest.js','src/services/gmail-access-token.js','src/calendar/providers/google.js','src/calendar/providers/apple-caldav.js','src/security/credential-crypto.js','src/jobs/reminders.js','src/jobs/calendar-sync.js','src/jobs/gmail-life-admin-backfill.js','src/auth/background-supabase.js'])assert.ok(fs.existsSync(path.join(__dirname,'..',file)),`missing ${file}`);
